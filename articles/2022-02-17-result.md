...
- 上記の回避策として、`flatMap()` 内で subscribe することなく `tryMap()` -> `catch` -> `Empty` の合わせ技をかけることによって、エラーがあっても止まらないストリームを作り出せる
...
ポイントは、エラーが発生しても止めないようにはどうするかです。

## （ダメな例）案1： AnyPublisher<Int, Error> -> tryMap() -> catch -> Empty の合わせ技
...
`flatMap()` 内に隠蔽する技として、`.collect(1)` や `.map { [$0] }` するという技があるのでそれも使います。
